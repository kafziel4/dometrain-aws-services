...
public class CustomerImageService : ICustomerImageService
{
    ...
    private readonly string _bucketName = "keomaawscourse";
    ...
    {
        var deletePbjectRequest = new DeleteObjectRequest
        {
            BucketName = _bucketName,
            Key = $"images/{id}"
        };

        return await _s3.DeleteObjectAsync(deletePbjectRequest);
    }
}